...
const logger = require('./logger.helper').Logger;
const { formatTime } = require('./common.helper');
async function responseLogger({ res, message, statusCode, payload = {} }) {
...
const response = {
    ...
    MULTIPLE_LOGIN({ res, message = 'Multiple login', payload = {} } = {}) {
        res.status(HTTP_CODES.MULTIPLE_LOGIN).json({
            success: false,
            message,
            payload,
        });
    },
    NOT_FOUND_FOR_FIRM({ res, message = 'Not found for firm', payload = {} } = {}) {
        res.status(HTTP_CODES.NOT_FOUND_FOR_FIRM).json({
            success: false,
            message,
            payload,
        });
...
for (const key in response) {
    ...
    response[key] = (args) => {
        responseLogger({ ...args, statusCode: HTTP_CODES[key], payload: args.payload })
        fn(args);
    }
}
